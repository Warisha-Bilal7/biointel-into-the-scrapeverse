...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchHealth() {
      try {
        const res = await fetch('/api/v1/scraper-health');
        if (!res.ok) throw new Error('Network error');
        const data = await res.json();
        setHealth(data as ScraperHealth);
      } catch (e) {
        console.error('Failed to fetch scraper health:', e);
      }
    }
  ...
  const deriveSources = health
    ? [
        {
          name: 'Clinical Data',
          status: health.structural_drift_score < 0.20 && health.semantic_drift_score < 0.35 ? 'Healthy' : 'Drift Detected',
          confidence: Math.round(100 - (health.structural_drift_score * 2 + health.semantic_drift_score * 1.5)),
        },
        {
          name: 'Drug Database',
          status: 'Healthy',
        ...
        {
          name: 'Research Source',
          status: health.quarantined_items > 0 ? 'Drift Detected' : 'Healthy',
          confidence: health.quarantined_items > 0 ? 42 : 98,
        },
      ]
    : SOURCES_DEFAULT;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex flex-col flex-1 overflow-auto">
        ...
        <main className="p-6 md:p-8">
          ...
          {health ? (
            <section
              className="mb-12 rounded-2xl p-6 md:p-8 border-t"
              style={{ borderColor: 'rgba(15, 107, 94, 0.3)' }}
            >
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-overflow">
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    ...
                    <h3 className="text-2xl font-semibold text-emerald-400">
                      {health.status === 'healthy' ? 'HEALTHY' : 'ANOMALOUS'}
                    </h3>
                  </div>
                ...
                <div
                  className="rounded-2xl bg-opacity-70 p-6 md:p-8 text-center"
                  style={{ background: 'rgba(15, 107, 94, 0.15)' }}
                >
                  <p className="text-xs text-slate-400 uppercase tracking-wider">DATA CONFIDENCE</p>
                  <p className="mt-2 text-4xl font-bold text-white">
                    {Math.round(
                      100 - (health.structural_drift_score * 2 + health.semantic_drift_score * 1.5)
                    )}%
                  </p>
                </div>
              </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {/* Sources */}
            <section
              className="lg:col-span-2 rounded-2xl p-6 md:p-8 bg-card border border-border/30"
            >
            ...
            {health && health.is_anomalous ? (
              ...
              >
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <p className="text-xs font-medium text-emerald-400 uppercase tracking-wider">NO ALERTS</p>
                    <h3 className="font-semibold text-emerald-400">SYSTEM HEALTHY</h3>
                  </div>
                </div>

                <div>
                  <p className="mb-3 text-sm font-medium text-slate-400/50">Data confidence</p>
                  <p className="mt-1 text-3xl font-bold text-emerald-400">97%</p>
                </div>
              </aside>
            )}
            ...
            <section className="mt-12 rounded-2xl p-6 md:p-8 bg-card border border-border/30">
              ...
              <div className="space-y-3">
                {health && health.recent_runs.length > 0
                  ? health.recent_runs.map((event, index) => (
                      <div
                      ...
                      >
                        <span className="w-10 font-mono text-opacity-60 text-sm">
                          {event.received_at?.split('T')[1]?.split('.')[0] || event.time}
                        </span>
